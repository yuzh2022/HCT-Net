...
def create_exp_dir(path, desc='Experiment dir: {}'):#写入实验路径
    if not os.path.exists(path):
        os.makedirs(path)
    print(desc.format(path))

def convert_state_dict(state_dict):#从一个数据平行模块转化到一个静态字典
    """Converts a state dict saved from a dataParallel module to normal
       module state_dict inplace :param state_dict is the loaded DataParallel model_state
    """

    new_state_dict = OrderedDict()  #使用OrderedDict会根据放入元素的先后顺序进行排序
...
def get_logger(log_dir):
    ...
    fh = logging.FileHandler(os.path.join(log_dir, 'run.log')) #写日志
