...
class BinaryIndicatorsMetric(object):
    ...
    def update(self,labels,preds,n=1,threshold=0.5):
        ...
        Inter = torch.sum(TP)
        # print("Inter:",preds + labels)
        Union = torch.sum(TP + FP + FN)
        # print("Union:",Union)
        self.jc = float(Inter) / (float(Union) + 1e-6)
        self.dice = float(torch.sum(2 * TP)) / (float(torch.sum(2 * TP + FP + FN)) + 1e-6)
    ...
    @property
    def get_avg(self):
        return self.Recall/self.length, self.Specificity/self.length,self.Precision/self.length,self.F1/self.length,self.Jc/self.length,\
...
if __name__=="__main__":
    ...
    image_sir=r'F:\820\subject\dataset\MixSearch\ISIC2018\Task1_apply\train\ISIC_0000001.png'
