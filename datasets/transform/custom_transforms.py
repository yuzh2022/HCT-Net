...
class RandomVerticallyFlip(object): #随机垂直翻转
    def __call__(self, img,mask):
        assert img.size == mask.size
        if random.random() < 0.5:
...
class FixedResize(object): #修改基准尺寸
...
class RandomScaleCrop(object): #随机尺度作物
...
class RandomElasticTransform(object): #随机弹性变换
